...
- 可以通过 `cargo doc` 命令生成 HTML 帮助文档：
  - 该命令会运行 Rust 安装包自带的 rustdoc 工具；
  - 会把生成的文档放在 *target/doc* 目录下。
- 可以通过 `cargo doc --open` 命令快捷生成并打开文档：
  - 构建当前 crate 的文档，包括依赖项的文档；
  - 在浏览器中打开文档。
- 支持 MarkDown 格式；
- 常用章节：
  - `# Examples`：运行 `cargo test` 的时候会将其中的事例代码作为测试来运行；
  - `Panic`：函数可能发生 panic 的场景；
  - `Errors`：如果函数返回 Result，描述可能的错误种类，以及会导致错误的条件；
  - `Safety`：如果函数处于 unsafe 调用，解释函数 unsafe 的原因，以及调用者确保的使用前提。
- 分为内部文档注释和外部文档注释：
  - 内部文档注释（Inner doc comment）
    - 单行注释（以 `///` 开头）；
    - 块注释（用 `/** ... */` 分隔）；
    - 内部文档注释是对它**之后的项**做注释，与使用 `#[doc="..."]` 是等价的。
  - 外部文档注释（Outer doc comment）
    - 单行注释（以 `//!` 开头）；
    - 块注释（用 `/*! ... */` 分隔）；
    - 外部文档注释是对它**所在的项**做注释，与使用 `#![doc="..."]` 是等价的；
    - 通常用来描述 crate 和模块。
